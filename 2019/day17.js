...
let path = "";
while (!done) {
  // first turn the robot ninety degrees
  // dir key:
  // [0,1] = right
  // [0,-1] = left
  // [1,0] = down
  // [-1,0] = up
  if (robotDir[0] === 0 && robotDir[1] === 1) {
    // facing right
    if (isBeam(robotPos[0] + 1, robotPos[1])) {
      // turn to face down
      path += 'R';
      robotDir = [1,0];
    } else if (isBeam(robotPos[0] - 1, robotPos[1])) {
      // turn to face up
      path += 'L';
      robotDir = [-1,0];
    } else {
  ...
  } else if (robotDir[0] === 0 && robotDir[1] === -1) {
    // facing left
    if (isBeam(robotPos[0] + 1, robotPos[1])) {
      // turn to face down
      path += 'L';
  ...
  } else if (robotDir[0] === 1 && robotDir[1] === 0) {
    // facing down
    if (isBeam(robotPos[0], robotPos[1] + 1)) {
      // turn to face right
      path += 'L';
      robotDir = [0,1];
    } else if (isBeam(robotPos[0], robotPos[1] - 1)) {
      // turn to face left
      path += 'R';
      robotDir = [0,-1];
    } else {
  ...
  let amt = 0;
  while(isBeam(robotPos[0] + robotDir[0], robotPos[1] + robotDir[1])) {
    amt++;
    robotPos = [robotPos[0] + robotDir[0], robotPos[1] + robotDir[1]];
  }
  path += `${amt},`;
}
console.log(path);
