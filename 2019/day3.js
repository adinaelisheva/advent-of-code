...
function hash(pos) {
  // While here, do some comparisons
...
function parseInstruction(inst) {
  const dir = inst[0];
  const amt = Number(inst.substring(1));
  // R by default
  let xy = 0; // 0 = x, 1 = y
  let incr = 1; // 1 for up/right, -1 for down/left
  switch(dir) {
    case 'L':
      incr = -1;
      break;
...
let pos = [0,0];
function updateSeenSpots(hashStr, dir) {
  let char = dir === 0 ? '-' : '|';
  seenSpots[hashStr] = char;
}
let instNum = 0;
wire1.forEach((inst) => {
  console.log(`parsing instruction ${instNum++} of ${wire1.length} in wire 1`);
  [amt, xy, incr] = parseInstruction(inst);
  for(let i = 0; i < amt; i++) {
    updateSeenSpots(hash(pos), xy);
    pos[xy] += incr;
  }
});
updateSeenSpots(hash(pos), xy);
...
function updateCrossings(dir) {
  const hashStr = hash(pos);
  const found = !!seenSpots[hashStr];
  updateSeenSpots(hashStr, dir);
  // 0,0 doesn't count as a crossing
  if (found && hashStr !== '0,0') {
...
instNum = 0;
wire2.forEach((inst) => {
  console.log(`parsing instruction ${instNum++} of ${wire2.length} in wire 2`);
  [amt, xy, incr] = parseInstruction(inst);
  for(let i = 0; i < amt; i++) {
    updateCrossings(xy);
    pos[xy] += incr;
...
for (let y = maxY; y >= minY; y--) {
  for (let x = minX; x <= maxX; x++) {
    let hashStr = hash([x, y]);
    let char = '.';
    if (seenSpots[hashStr]) {
      char = seenSpots[hashStr];
    }
    if (x === 0 && y === 0) { char = 'o'; }
    output += char;
  }
