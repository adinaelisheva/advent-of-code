...
let grid, rPos, rX, rY, minX, maxX, minY, maxY;
let paintedPanels = 0;
...
function runRobot() {
  ...
  while (program[ip] !== 99) {
    ...
    switch (opcode) {
      ...
      case 3:
        advAmt = 2;
        const index = modes[0] === REL_MODE ? a + relativeBase : a;
        program[index] = grid[`${rPos[0]},${rPos[1]}`] ? 1 : 0;
        break;
      case 4:
        advAmt = 2;
        if (outputtingColor) {
        ...
        outputtingColor = !outputtingColor;
        break;
      case 5:
        if (valA) {
          ip = valB;
...
reset();
runRobot();
console.log(`Painted ${Object.keys(grid).length} panels`);

reset();
grid[`${rPos[0]},${rPos[1]}`] = 1;
runRobot();
