...
for(var i = 0; i < input.length; i++) {
  var m;
  if(m = startRE.exec(input[i])) {
    var value = m[1];
    var id = m[2];
    addNumToBot(value,id);
  }
  else if(m = giveRE.exec(input[i])) {
    var id = m[1];
    instructions[id] = m;
  }
}
...
while(botsToHandle.length > 0) {
  ...
  var low = Math.min(bot[0],bot[1]);
  var high = Math.max(bot[0],bot[1]);
  var lowId = inst[3];
  var highId = inst[5];

  if(inst[2] === 'bot') {
    addNumToBot(low,lowId);
  } else {
    outputs[lowId] = Number(low);
  }

  if(inst[4] === 'bot') {
    addNumToBot(high,highId);
  } else {
    outputs[highId] = Number(high);
  }
